...
public class GsonHttpMessageConverter extends AbstractHttpMessageConverter<Object> {
    ...
    public GsonHttpMessageConverter(Map<Class, Object> customTypeAdapters) {
        super(new MediaType("application", "json", DEFAULT_CHARSET));
        Assert.notNull(customTypeAdapters, "constructor argument 'customTypeAdapters' is null");
        GsonBuilder gb = new GsonBuilder().disableHtmlEscaping();
        for (Class clazz : customTypeAdapters.keySet()) {
            gb = gb.registerTypeAdapter(clazz, customTypeAdapters.get(clazz));
        }
        setGson(gb.create());
    }
    ...
    protected void writeInternal(Object o, HttpOutputMessage outputMessage) throws IOException {
        try (OutputStreamWriter writer = new OutputStreamWriter(outputMessage.getBody(), getCharset(outputMessage.getHeaders()))) {
            if (prefixJson) {
                writer.append("{} && ");
            }
            Type typeOfSrc = type;
